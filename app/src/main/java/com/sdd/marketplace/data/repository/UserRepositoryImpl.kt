...
class UserRepositoryImpl @Inject constructor(
    ...
    override suspend fun updateProfile(updates: Map<String, Any>): Result<User> = runCatching {
        ...
        val jsonUpdate = kotlinx.serialization.json.buildJsonObject {
            updates.forEach { (key, value) ->
                when (value) {
                    ...
                    is Double -> put(key, kotlinx.serialization.json.JsonPrimitive(value))
                    is Float -> put(key, kotlinx.serialization.json.JsonPrimitive(value))
                    else -> put(key, kotlinx.serialization.json.JsonPrimitive(value.toString()))
                }
            }
        }
        postgrest["users"].update(jsonUpdate) { filter { eq("id", userId) } }
        val updated = postgrest["users"].select { filter { eq("id", userId) } }.decodeSingleOrNull<UserDto>()
        updated?.let { userDao.insertUser(it.toEntity()) }
        updated?.toDomain() ?: throw Exception("Profile update failed")
    }
    ...
    override suspend fun followUser(userId: String): Result<Unit> = runCatching {
        ...
        runCatching {
            postgrest.rpc("increment_follower_count", buildJsonObject {
    ...
    override suspend fun unfollowUser(userId: String): Result<Unit> = runCatching {
        ...
        runCatching {
            postgrest.rpc("decrement_follower_count", buildJsonObject {
                put("target_user_id", userId)
            })
        }
    }
    ...
    override suspend fun uploadAvatar(uri: Uri): Result<String> = runCatching {
        ...
        storage.from(BuildConfig.BUCKET_AVATARS).upload(fileName, bytes) {
            upsert = true
        }
        val url = storage.from(BuildConfig.BUCKET_AVATARS).publicUrl(fileName)
